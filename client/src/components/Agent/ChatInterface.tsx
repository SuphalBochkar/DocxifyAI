...
export function ChatInterface({ messages, onSendMessage, isLoading = false, selectedDocument }: ChatInterfaceProps) {
    ...
    <Card className="flex flex-col h-full border-slate-200 shadow-sm overflow-hidden">
      <CardHeader className="px-4 py-3 border-b bg-slate-50">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-medium text-slate-800">AI Assistant</CardTitle>
          {selectedDocument && (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((message) => (
          <div
            key={message.id}
            className={cn("flex items-start gap-3", message.role === "assistant" ? "" : "justify-end")}
          >
            ...
            >
              <div className="mb-1 font-medium">{message.role === "assistant" ? "AI Assistant" : "You"}</div>
              <div className="whitespace-pre-wrap">{message.content}</div>
              <div className="mt-1 text-xs opacity-70">{new Date(message.timestamp).toLocaleTimeString()}</div>
            </div>
            {message.role === "user" && (
              <Avatar className="h-8 w-8 mt-1">
                <AvatarFallback className="bg-slate-200 text-slate-800">You</AvatarFallback>
              </Avatar>
            )}
          </div>
        ))}
      ...
      <CardContent className="p-4 border-t mt-auto">
        <form onSubmit={handleSubmit} className="flex gap-2">
          ...
          <Button type="submit" disabled={!input.trim() || isLoading || !selectedDocument} className="shrink-0">
            {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <Send className="h-5 w-5" />}
            <span className="sr-only">Send message</span>
          </Button>
        </form>
        {!selectedDocument && (
          <p className="text-sm text-slate-500 mt-2 text-center">Please select a document to start a conversation</p>
        )}
      </CardContent>
    </Card>
  )
